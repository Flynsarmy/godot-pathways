@tool
@icon("res://addons/pathways/icons/pathway-network.png")

extends Node3D
class_name PathwayNetwork
...
var split_curves : bool = false: set = set_split_curves
## Number of curve points before a curve is split.
var split_size : int = 8: set = set_split_size
...
func _property_can_revert(property: StringName) -> bool:
	if (property == "split_curves"):
		return split_curves != false

	if (property == "split_size"):
		return split_size != 8
...
func _iter_check_if_intersection_explored(node_id: int) -> bool:
	...
	for nid in node_data.next:
		if (_iterator_unvisited_ids.has(nid)):
			return false
...
func _update_intersections() -> void:
	...
	for node_id in network_nodes:
		...
		if (branch_count > 2):
			...
			curves.sort_custom(Callable(self, "_sort_intersection_curves"))
...
func _get_ordered_node_controls(node_data: Dictionary, from_control_index: int) -> Array:
	...
	unsorted_angles.sort_custom(Callable(self, "_sort_node_controls"))
	var sorted_indices := []
...
func _rotate_handle_around_node(from_position: Vector3, degrees: float) -> Vector3:
	var rotated_position = from_position.rotated(Vector3(0, 1, 0), deg_to_rad(degrees))
...
func _print_verbose(message: String = "") -> void:
	if (!enable_verbose_print || !Engine.is_editor_hint()):
		return
