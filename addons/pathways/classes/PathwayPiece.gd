@tool
@icon("res://addons/pathways/icons/pathway-piece.png")

extends Node3D
class_name PathwayPiece
...
# Public properties
@export var mesh_node : NodePath: set = set_mesh_node
@export var piece_type: PieceType = PieceType.STRAIGHT: set = set_piece_type

# Dynamically defined properties
## Intersection properties
var intersection_type : int = IntersectionType.Y: set = set_intersection_type
var intersection_branch_length : float = 1.0: set = set_intersection_branch_length
var intersection_branch_radius : float = 1.0: set = set_intersection_branch_radius
var intersection_origin_point : Vector2 = Vector2.ZERO: set = set_intersection_origin

## Mesh adjustments
var mesh_axis : String = "X": set = set_mesh_axis
var mesh_up_axis : String = "Y": set = set_mesh_up_axis
var mesh_rotation_degrees : float  = 0.0: set = set_mesh_rotation
...
func _get_property_list() -> Array:
	...
	var intersection_usage := PROPERTY_USAGE_NO_EDITOR
...
func _update_branches() -> void:
	...
	if (intersection_type == IntersectionType.T):
		...
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(90)))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(-90)))

	elif (intersection_type == IntersectionType.Y):
		_intersection_branches.append(Vector2(0, -intersection_branch_length))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(120)))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(-120)))

	elif (intersection_type == IntersectionType.X):
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(45)))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(135)))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(-135)))
		_intersection_branches.append(Vector2(0, -intersection_branch_length).rotated(deg_to_rad(-45)))
...
func _update_mesh() -> void:
	...
	var mesh_instance = get_node_or_null(mesh_node) as MeshInstance3D
...
func _pretransform_straight(input_mesh: ArrayMesh) -> ArrayMesh:
	...
	for i in mesh_data_tool.get_vertex_count():
		...
		position = pre_transform * (position)
		normal = pre_transform * (normal)
...
func _get_pre_transform() -> Transform3D:
	var rotation_angle = deg_to_rad(mesh_rotation_degrees)
