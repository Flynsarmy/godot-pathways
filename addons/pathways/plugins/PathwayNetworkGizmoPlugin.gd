...
var _intersection_ids : PackedInt32Array
...
func _init(editor_plugin: EditorPlugin) -> void:
	...
	create_material("network_lines", editor_theme.get_color("network_lines", "PathwaysPlugin"))
	create_material("control_lines", editor_theme.get_color("control_lines", "PathwaysPlugin"))
	create_material("intersection_lines", editor_theme.get_color("intersection_lines", "PathwaysPlugin"))
	create_material("baked_lines", editor_theme.get_color("baked_lines", "PathwaysPlugin"))
	_create_custom_handle_material("node_handles", editor_theme.get_icon("node_handle", "PathwaysPlugin"), -3)
	_create_custom_handle_material("node_selected_handles", editor_theme.get_icon("node_handle_selected", "PathwaysPlugin"), 0)
	_create_custom_handle_material("node_branch_handles", editor_theme.get_icon("node_handle_add", "PathwaysPlugin"), -4)
	_create_custom_handle_material("control_handles", editor_theme.get_icon("control_handle", "PathwaysPlugin"), -5)
	_create_custom_handle_material("intersection_handles", editor_theme.get_icon("intersection_handle", "PathwaysPlugin"), -5)
...
func _has_gizmo(for_node_3d: Node3D) -> bool:
	return for_node_3d is PathwayNetwork
...
func _get_handle_name(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool) -> String:
	var network = gizmo.get_node_3d() as PathwayNetwork
	var gizmo_node = _get_gizmo_node(handle_id)
...
func _set_handle(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool, camera: Camera3D, screen_pos: Vector2) -> void:
	...
	if (control_pressed && !gizmo_node.is_control):
		...
		var next_position = _intersect_screen_space(camera, screen_pos, node_data.position + node_data.branch_control)
		if (next_position):
	...
	var next_position = _intersect_screen_space(camera, screen_pos, position_origin)
	if (next_position):
		next_position = network.to_local(next_position)
...
func _commit_handle(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool, restore: Variant, cancel: bool) -> void:
	...
	var current_value = _get_handle_value(gizmo, handle_id, secondary)
...
func _redraw(gizmo: EditorNode3DGizmo) -> void:
	...
	if (network == _edited_network):
		if (gizmo_data.pre_selected_handles.size() > 0):
			gizmo.add_handles(
				gizmo_data.pre_selected_handles,
				get_material("node_handles", gizmo),
				[]
			)
		if (gizmo_data.selected_handles.size() > 0):
			gizmo.add_handles(
				gizmo_data.selected_handles,
				get_material("node_selected_handles", gizmo),
				[]
			)
		if (gizmo_data.post_selected_handles.size() > 0):
			gizmo.add_handles(
				gizmo_data.post_selected_handles,
				get_material("node_handles", gizmo),
				[]
			)
		...
		for control_handles in gizmo_data.control_handles:
			if (control_handles.size() > 0):
				gizmo.add_handles(
					control_handles,
					get_material("control_handles", gizmo),
					[]
				)

		if (gizmo_data.intersection_handles.size() > 0):
			gizmo.add_handles(
				gizmo_data.intersection_handles,
				get_material("intersection_handles", gizmo),
				[]
			)

		if (gizmo_data.branch_handles.size() > 0):
			gizmo.add_handles(
				gizmo_data.branch_handles,
				get_material("node_branch_handles", gizmo),
				[]
			)
...
func draw_over_viewport(overlay: Control) -> void:
	...
	var handle_icon = base_theme.get_icon("node_handle_add", "PathwaysPlugin")
	if (!handle_icon):
		return
...
func handle_gui_input(camera: Camera3D, event: InputEvent) -> bool:
	...
	if (mb):
		...
		if (mb.button_index == MOUSE_BUTTON_LEFT && mb.ctrl_pressed):
			...
			if (_edited_network.network_nodes.size() > 0):
				...
				origin = gt * (last_node.position)

			var intersects = _intersect_screen_space(camera, mb.position, origin)
			if (intersects):
				var next_node_id = _edited_network.get_next_id()
				var next_node_position = it * (intersects)
	...
	if (mm):
		if (_get_node_at_position(camera, mm.position) >= 0):
			...
			_preview_placement = mm.ctrl_pressed
...
func _get_gizmo_data(network: PathwayNetwork) -> Dictionary:
	...
	var pre_selected_handles := PackedVector3Array()
	var selected_handles := PackedVector3Array()
	var post_selected_handles := PackedVector3Array()

	var control_handles := []
	var intersection_handles := PackedVector3Array()
	var branch_handles := PackedVector3Array()
	var control_lines := PackedVector3Array()
	var intersection_lines := PackedVector3Array()
	var branch_lines := PackedVector3Array()

	var network_lines := PackedVector3Array()
	var baked_lines := PackedVector3Array()
	...
	for node_id in network.network_nodes:
		...
		# node indices later.
		var node_control_handles := PackedVector3Array()
...
func _get_node_at_position(camera: Camera3D, position: Vector2, with_controls: bool = false) -> int:
	...
	for node_id in _edited_network.network_nodes:
		...
		var distance = camera.unproject_position(gt * (node_data.position)).distance_to(position)
		...
		if (with_controls):
			var control_index := 0
			for in_control in node_data.in_controls:
				var in_distance = camera.unproject_position(gt * (node_data.position + in_control)).distance_to(position)
			...
			for out_control in node_data.out_controls:
				var out_distance = camera.unproject_position(gt * (node_data.position + out_control)).distance_to(position)
...
func _get_path_at_position(camera: Camera3D, position: Vector2) -> Array:
		...
		for i in curve.get_point_count() - 1:
			...
			while (j < curve_points.size() && next_position != curve_points[j]):
				...
				if (j_distance > 0):
					...
					var segment_point = Geometry2D.get_closest_point_to_segment(position, segment_from, segment_to)
					...
					if (click_distance < CLICK_DISTANCE && click_distance < closest_distance):
						...
						var ray_results = Geometry3D.get_closest_points_between_segments(ray_from, ray_from + ray_direction * RAY_DISTANCE, j_from, j_to)

						closest_position = it * (ray_results[1])
...
func _intersect_with_colliders(camera: Camera3D, screen_point: Vector2): # Vector3 or null
	var from = camera.project_ray_origin(screen_point)
	var dir = camera.project_ray_normal(screen_point)

	var space_state = _edited_network.get_world_3d().direct_space_state
	var query = PhysicsRayQueryParameters3D.create(from, from + Vector3(0, -10, 0))
...
func _intersect_with_plane(camera: Camera3D, screen_point: Vector2) -> Vector3:
	var from = camera.project_ray_origin(screen_point)
	var dir = camera.project_ray_normal(screen_point)
...
func _intersect_screen_space(camera: Camera3D, screen_point: Vector2, from_origin: Vector3) -> Vector3:
	var from = camera.project_ray_origin(screen_point)
	var dir = camera.project_ray_normal(screen_point)

	var gt = _edited_network.get_global_transform()
	var point: Vector3 = gt * (from_origin)
	var camera_basis: Basis = camera.get_transform().basis
	var plane := Plane(point, point + camera_basis.x, point + camera_basis.y)

	return plane.intersects_ray(from, dir)

func _create_custom_handle_material(name: String, icon: Texture2D, priority: int = 0, billboard: bool = false) -> void:
	...
	material.render_priority = StandardMaterial3D.RENDER_PRIORITY_MAX + priority
